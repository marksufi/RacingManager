 ...
 */
public class ListMatrixMathematics {

    public static Matrix transpose(MatrixList matrix) {
        int ncols = matrix.getNcols();
        int nrows = matrix.getNrows();

        Matrix transposedMatrix = new Matrix(ncols, nrows);
    ...
    public static Matrix inverse(Matrix matrix) throws NoSquareException {
        Matrix cm = cofactor(matrix);
        Matrix tm = transpose(cm);

        return tm.multiplyByConstant(1.0/determinant(matrix));
    }
    ...
    public static double determinant(Matrix matrix) throws NoSquareException {
        ...
        for (int i=0; i<matrix.getNcols(); i++) {
            double d = matrix.getValueAt(0, i);
            Matrix sub = createSubMatrix(matrix, 0, i);
    ...
    public static Matrix multiply(Matrix matrix1, MatrixList matrix2)  {
